...
(define-class human%
  ...
  (define/public (on-key key)
    (cond [(string=? "up" key) (new human% 
                                    (send (field bike1) change-dir key) 
                                    (field bike2))]
          [(string=? "down" key) (new human%  
                                      (send (field bike1) change-dir key) 
                                      (field bike2))]
          [(string=? "left" key) (new human% 
                                      (send (field bike1) change-dir key) 
                                      (field bike2))]
          [(string=? "right" key) (new human% 
                                       (send (field bike1) change-dir key) 
                                       (field bike2))]
          [else this]))
...
;;A Computer is a (new computer% Number Bike Bike)
(define-class computer%
  ...
  ;;Sends a bundle containing data for movement processing when it ticks
  (define/public (on-tick)
    (if (= 0 (modulo (field ticks) 2))
        (make-package this (send this random-dir))
        (make-package this (send (field bike1) dir))))
  
  ;;Random-dir: -> Direction
  ;;Returns a random direction (that won't lead you backwards)
  (define/public (random-dir)
    (local [(define generated (random 4))
            (define currDir (send (field bike1) dir))]
      (cond[(and (= generated 0)
                 (not (string=? currDir "down")))
            "up"]
           [(and (= generated 1)
                 (not (string=? currDir "up")))
            "down"]
           [(and (= generated 2)
                 (not (string=? currDir "left")))
            "right"]
           [(and (= generated 3)
                 (not (string=? currDir "right")))
            "left"]
           [else currDir])))
  )
...
(define-class bike%
  ...
  (define/public (collide? bike)
    (cond[(member? (field loc) (send bike trail)) true]
         ...
         [(equal? (field loc) (send bike loc)) true]
...
;;Parse: [ListOf SExprs] -> [ListOf Bike]
;;Puts together bikes out of a list of them
(define (parse m)
  (list (new bike% 
             (first (first m))
             (first (rest (first m)))
...
(check-expect (parse (cons (list 1 2 3 4)
                           (list 1 2 3 4)))
              (list (new bike% 1 2 3 4)
                    (new bike% 1 2 3 4)))


;;Tests for the World% class
(define TESTWORLD (new human%
...
(check-expect (send TESTWORLD on-key "left")
              (new human% (new bike% 1+1i "blue" empty "left")
                   (new bike% 2+2i "orange" empty "up")))
(check-expect (send TESTWORLD on-key "up")
              (new human% (new bike% 1+1i "blue" empty "up")
                   (new bike% 2+2i "orange" empty "up")))
...
(launch-many-worlds 
 ...
 (big-bang (new human% BIKE1DEFAULT BIKE2DEFAULT))
 (big-bang (new computer% 0 BIKE2DEFAULT BIKE1DEFAULT)))